...
def test_chunker_splits_long_text():
    ...
    # Use text with periods so sentence splitting works properly
...
def test_chunker_respects_chunk_size():
    ...
    # Use text with periods so sentence splitting works - each sentence is within chunk_size
    text_parts = [". ".join(["sentence number " + str(i)] * 20) + "." for i in range(50)]
    text = " ".join(text_parts)
    chunks = chunker.chunk(text, {"source_file_name": "test.txt"})
    for chunk in chunks:
        assert len(chunk.content) <= 600  # allow some buffer for overlap
